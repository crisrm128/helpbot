...
class ClienteMoveBase:
    ...
    def moveTo(self, x, y, qx, qy, qz, qw):
        ...
        #La orientación es un quaternion. 
...
if __name__ == "__main__":
    if len(sys.argv) <= 2:
        print("Uso: " + sys.argv[0] + " x_objetivo y_objetivo qx_objetivo qy_objetivo qz_objetivo qw_objetivo")
        exit()      
    rospy.init_node('prueba_clientemovebase')
    cliente = ClienteMoveBase()
    result = cliente.moveTo(float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[4]), float(sys.argv[5]), float(sys.argv[6]), float(sys.argv[7]))
    print(result)
    if result:
